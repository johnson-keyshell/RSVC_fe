...
function AgentBuyerContact({ contacts, changeChat }) {
        ...
        <div >
          ...
          {contacts.length != 0 ? (
            <div style={{ minHeight:"22vh", height: "fit-content", overflowY: 'auto', maxHeight: "34vh" }}>
              <ul>
                {contacts.filter(contact => contact.chatStatus === 'Active')
                .map((contact) => (
                  ...
                  <div className="thumbnail-text">{contact?.partner.name}</div>
                      ...
                      <div className="contact-details-doc">
                        ...
                        <p className="agent-name-seller">{t("agent")}:{contact?.agent?.name}</p>
                      </div>
...
export default AgentBuyerContact;
